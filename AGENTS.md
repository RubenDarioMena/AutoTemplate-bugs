...
- La aplicación canónica es `bug_tool.html`: un monolito HTML autocontenido.
- `#vanillaConfig` dentro de ese archivo es la configuración distribuida. `config` es la copia editable local y `state` contiene las instancias de trabajo.
...
- `Directorio-Monolito.MD` es el mapa oficial de secciones y dependencias del monolito.

## Regla obligatoria de documentación

Todo cambio estructural en `bug_tool.html` debe actualizar `Directorio-Monolito.MD` dentro de la misma tarea. Se considera estructural cualquier cambio que:
...
Después de cambiar `bug_tool.html`:
